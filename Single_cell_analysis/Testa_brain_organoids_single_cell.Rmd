...
## Do normalization separately for each dataset
...
```{r}
anchors <- FindIntegrationAnchors(object.list = merged_seurat_integrated.list, anchor.features = features , reduction = "rpca",
    dims = 1:40)
merged_seurat_integrated <- IntegrateData(anchorset = anchors, dims = 1:40)
```
## Run the standard workflow for visualization
```{r}
DefaultAssay(merged_seurat_integrated) <- "integrated"
merged_seurat_integrated <- ScaleData(merged_seurat_integrated, verbose = FALSE)
merged_seurat_integrated <- RunPCA(merged_seurat_integrated, verbose = FALSE)
merged_seurat_integrated <- RunUMAP(merged_seurat_integrated, dims = 1:50)
```
```{r}
## Save the integrated Seurat object
...
merged_seurat_integrated <- FindClusters(merged_seurat_integrated, resolution = 0.42)
