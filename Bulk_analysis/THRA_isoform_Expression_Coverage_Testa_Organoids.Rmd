...
## Lets normalize by the M of uniquely aligned reads
data_stringtie_filtered_normalized <- inner_join(data_stringtie_filtered, depth, by ="Sample")

data_stringtie_filtered_normalized <- data_stringtie_filtered_normalized %>% 
  mutate(across(where(is.numeric), ~ .x/`M Aligned`)) # value/M of reads

### let's fill in with the metadata
data_stringtie_filtered_normalized <- inner_join(data_stringtie_filtered_normalized, metadata, by ="Sample") %>%
  select(-`M Aligned`, -`% Aligned`) %>%
  unique() # remove duplicates coming from paired reads
```
...
data_stringtie_filtered_normalized
  
```

```{r}
total_plot <-  select(data_stringtie_filtered_normalized, Sample, study_day,  THRA1, THRA2)
...
ggsave("THRA1_THRA2_mosdepth_over_time_organoids_stacked_with_error_bars_STRINGTIE.png" , device=png, dpi = 600, path = path_plots, bg = "transparent", width = 20, height = 15, units = "cm" )
```
